...
public static class ActiveDirectoryExtensions
{
    ...
    public static List<DirectoryAttribute> ToDirectoryAttributes(this IDictionary<string, List<string>> directoryAttributes)
    {
        ...
        foreach (var kvp in directoryAttributes) list.Add(kvp.Key, kvp.Value.ToArray());
    ...
    public static List<ActiveDirectoryObject> GetAll(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects(null, queryConfig, useCache);

    /// <summary>
    /// Gets all users in the Active Directory.
    /// </summary>
    /// <returns>A list of all users in the Active Directory.</returns>
    public static List<ActiveDirectoryObject> GetUsers(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(&(objectCategory=person)(objectClass=user))", queryConfig, useCache);

    /// <summary>
    /// Gets all OUs in the Active Directory.
    /// </summary>
    /// <returns>A list of all OUs in the Active Directory.</returns>
    public static List<ActiveDirectoryObject> GetOUs(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(objectCategory=organizationalUnit)", queryConfig, useCache);
    ...
    /// <returns>Returns a list of all users that were during the specified period of time.</returns>
    // ReSharper disable StringLiteralTypo
    public static List<ActiveDirectoryObject> GetUsersByLastLogonTimestampNull(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(&(objectCategory=person)(objectClass=user)(!lastlogontimestamp=*))", queryConfig, useCache);
    // ReSharper restore StringLiteralTypo
    ...
    public static List<ActiveDirectoryObject> GetComputers(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(&(objectCategory=computer)(objectClass=computer))", queryConfig, useCache);

    /// <summary>
    /// Gets all groups in the Active Directory.
    /// </summary>
    /// <returns>A list of all groups in the Active Directory.</returns>
    public static List<ActiveDirectoryObject> GetGroups(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(objectClass=group)", queryConfig, useCache);

    /// <summary>
    /// Gets all empty groups in the Active Directory.
    /// </summary>
    /// <returns>A list of all groups in the Active Directory.</returns>
    public static List<ActiveDirectoryObject> GetGroupsEmpty(this ActiveDirectoryCore ad, LdapQueryConfig queryConfig = null, bool useCache = false) => ad.GetObjects("(&(objectClass=group)(!member=*))", queryConfig, useCache);


    public static string DistinguishedName(this DirectoryEntry entry) => entry.GetString("distinguishedName");

    public static string GetString(this DirectoryEntry entry, string key) => entry.GetStrings(key).TrimOrNull().WhereNotNull().FirstOrDefault();

    public static IEnumerable<string> GetStrings(this DirectoryEntry entry, string key) => entry.GetObjects(key).Select(o => o.ToStringGuessFormat());
