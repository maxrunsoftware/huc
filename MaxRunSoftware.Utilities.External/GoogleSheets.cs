...
public class GoogleSheets : IDisposable
{
    ...
    private T IssueRequest<T>(SheetsBaseServiceRequest<T> request, string sheetName, string beforeMessage, string afterMessage, bool logResponse = true) where T : IDirectResponseSchema
    {
    ...
    public void ClearSheet(string sheetName)
    {
        ...
        FormatCellsDefault(sheetName, 0, 0, DefaultSheetWidth, DefaultSheetHeight);
    ...
    public void SetData(string sheetName, Table table, int characterThreshold)
    {
        ...
        FormatCellsDefault(sheetName, 0, 0, table.Columns.Count, table.Count + 1);
    ...
    public List<string[]> Query(string sheetName, string range = "A1:ZZ")
    {
        ...
        var rangeString = sheetName + "!" + (range ?? "A1:ZZ");
        var request = service.Spreadsheets.Values.Get(spreadsheetId, rangeString);

        var response = IssueRequest(request, sheetName, $"Querying {rangeString}", $"Queried {rangeString}", false);
