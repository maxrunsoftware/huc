...
public class WindowsTaskSchedulerBatchSync : WindowsTaskSchedulerBase
{
    ...
    private List<BatchFile> GetBatchFiles(IEnumerable<string> directoriesToScan)
    {
        ...
        foreach (var dir in directoriesToScan)
        {
            ...
            var batchFilesInDir = Util.FileListFiles(dir).Where(BatchFile.IsBatchFile).Select(o => new BatchFile(o, batchKeyword)).ToList();
    ...
    private void TaskCreate(WindowsTaskScheduler scheduler, WindowsTaskSchedulerPath path, BatchFile batchFile, string taskUsernameNew, string taskPasswordNew)
    {
        try
        {
            scheduler.TaskAdd(path.Add(batchFile.TaskName), batchFile.FilePath.Yield().ToArray(), batchFile.Triggers
                ...
                , username: taskUsernameNew
                , password: taskPasswordNew
            );
