...
public abstract class FtpBase : Command
{
    ...
    protected override void CreateHelp(CommandHelpBuilder help)
    {
        ...
        help.AddParameter(nameof(encryptionMode), "e", "Encryption Mode. NONE is for standard FTP. SSH is for SFTP. EXPLICIT and IMPLICIT are for FTPS " + DisplayEnumOptions(FtpEncryptionMode.None));
        help.AddParameter(nameof(encryptionProtocol), "s", "FTPS encryption protocol " + DisplayEnumOptions(SslProtocols.None));
...
*/
    ...
    private IFtpClient OpenClientSFtp()
    {
        ...
        {
            log.Debug($"Connecting to SFTP server {host}:{port} with username {username} with {keyFiles.Count} private keys");
            c = new FtpClientSFtp(host, port, username, keyFiles);
        }
